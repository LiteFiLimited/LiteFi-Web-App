  ...
  try {
    ...
    if (!/^\d{6}$/.test(code)) {
      return createErrorResponse(
        "Invalid verification code format. Must be a 6-digit code."
      );
    }
    ...
    try {
      ...
      if (backendResponse.ok) {
        return createSuccessResponse(
          backendData.message || "Email verified successfully",
          backendData.data
        );
      } else {
        return createErrorResponse(
      ...
      if (fetchError.name === "AbortError") {
        return createErrorResponse(
          "Backend request timed out. Please check if the backend server is running.",
          504
        );
      } else if (
        ...
        return createErrorResponse(
          "Cannot connect to backend server. Please verify the backend is running on the correct port.",
          503
        );
      } else {
        return createErrorResponse(
          `Backend connection failed: ${fetchError.message}`,
          502
        );
      }
    }
