  ...
  try {
    ...
    if (!VALID_EMPLOYMENT_STATUSES.includes(body.employmentStatus)) {
      return createErrorResponse(
        `Employment status must be one of: ${VALID_EMPLOYMENT_STATUSES.join(
          ", "
        )}`,
        400
      );
    ...
    if (body.employmentStatus !== "EMPLOYED") {
      ...
      const backendResponse = await fetch(`${apiUrl}/users/employment`, {
        method: "PATCH",
        headers: {
    ...
    if (
      ...
      (isNaN(body.monthlySalary) || body.monthlySalary <= 0)
    ) {
